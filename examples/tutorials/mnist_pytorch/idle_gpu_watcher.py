...
class IdleGpuException(Exception):
...
class IdleGpuWatcher:
    ...
    def _check_idle(self):
        ...
        for gpu_uuid, gpu_util in gpu_utils.items():
            ...
            if all(s < self._min_threshhold for s in self._gpu_util_samples[gpu_uuid]):
                raise IdleGpuException(
                    f"{gpu_uuid} usage below threshhold {self._min_threshhold}% "
                    f"for last {self._sample_freq * self._num_samples} seconds.")
            if self._debug:
                self._p(f"DEBUG: {gpu_uuid} usage {gpu_util}% ")

            self._gpu_util_samples[gpu_uuid].popleft()

    def _p(self, msg):
        print(f"IDLE_GPU_WATCHER: {msg}")
    ...
    def watch_process(self, args):
        ...
        except IdleGpuException as ex:
            sys.stderr.write(f"{ex}\n")

        finally:
            if p.poll() is None:
                self._p("Process still alive, killing.")
...
MIN_PERCENTAGE=1   # Average GPU usage which triggers a failure.
